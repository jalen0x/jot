class ExchangeRateCatalog
  ...
  def latest_rates(user:)
    ...
    rates = user.user_custom_exchange_rates.kept.order(:currency_code).map do |exchange_rate|
      Rate.new(currency_code: exchange_rate.currency_code, rate: exchange_rate.rate.to_s("F"))
    end
    rates << Rate.new(currency_code: base_currency_code, rate: BASE_RATE)

    Catalog.new(base_currency_code: base_currency_code, exchange_rates: rates.sort_by(&:currency_code))
  end
