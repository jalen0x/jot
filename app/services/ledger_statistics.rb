class LedgerStatistics
  ...
  class Result
    attr_reader :income_cents, :expense_cents, :net_cents, :amounts, :category_totals, :account_totals

    def initialize(income_cents:, expense_cents:, net_cents:, amounts:, category_totals:, account_totals:)
      @income_cents = income_cents
      @expense_cents = expense_cents
      @net_cents = net_cents
      @amounts = amounts
