# Transaction Template Order API Slice Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Allow the modern Rails transaction template update API to change `display_order`.

**Architecture:** Keep the Rails-native `PATCH /api/v1/transaction_templates/:id` endpoint as the update boundary. Permit `display_order` in the existing nested `transaction_template` params and assign it through `TransactionTemplateUpdater`, so clients can reorder templates without adding ezBookkeeping legacy `.json` move routes, `success/result` envelopes, camelCase params, or old frontend compatibility.
...
- Modify `app/controllers/api/v1/transaction_templates_controller.rb`: permit `display_order` in template params.
- Modify `app/services/transaction_template_updater.rb`: assign `display_order` when present in update params.
- Modify `test/integration/api/v1/transaction_templates_test.rb`: extend the existing update HTTP contract to prove `display_order` updates and is returned.
...
In `test "updates an owned transaction template and replaces tags"`, add `display_order: "5"` to the request params:

```ruby
          display_order: "5",
```

Change assertions to expect display order 5:
...
In `app/controllers/api/v1/transaction_templates_controller.rb`, add `:display_order` to `transaction_template_params`.

- [ ] **Step 2: Assign `display_order` in updater**

In `app/services/transaction_template_updater.rb`, add conditional assignment in `template_attributes`:
...
git commit --no-gpg-sign -m "feat: allow ordering transaction templates"
```
