...
- Shared non-secret test defaults may live in committed `.env.test`. Local runtime secrets live in ignored dotenv files such as `.env.development` or `.env.test.local`. One-time migration exports live under `tmp/` and are not loaded automatically.
